...
for i, folder in enumerate(experiments_folders):
    ...
    data["alg_name"] = algorithm_name[i]
    all_data = pd.concat([all_data, data])

# Apply Savitzky-Golay filter for smoothing
# all_data["log_relative_error_mean"] = all_data.groupby("alg_name")[
...
# all_data["K_abs_label"] = all_data["K_abs"].apply(lambda x: f"$K^{{abs}}={x}$")
...
for i, (label, style, color) in enumerate(
    zip(all_data["alg_name"].unique(), line_styles, line_colors)
):
    sns.lineplot(
        data=all_data[all_data["alg_name"] == label],
        x="Step",
...
plt.savefig(
    os.path.join(
        f"compare_two_stage_q_learning_performance_plt.png",
    ),
    bbox_inches="tight",
)
plt.savefig(
    os.path.join(f"compare_two_stage_q_learning_performance_plt.eps"),
    format="eps",
    bbox_inches="tight",
)
plt.show(block=True)
